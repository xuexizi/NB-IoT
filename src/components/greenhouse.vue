<template>
  <div>
    <el-row>
      <el-col :span="8" v-for="list in listGreen" :key="list.greenHouseName" style="padding: 10px">
        <el-card :body-style="{ padding: '0px' }">
          <div style="padding: 14px;">
            <span>{{list.greenHouseName}}</span>
            <div class="bottom clearfix">
              <el-button type="text" class="button" @click="del_warm(list)" style="color: red">删除</el-button>
              <el-button type="text" class="button" @click="chg_warm(list)">编辑</el-button>
              <el-button type="text" class="button" @click="chg_gh(list)">查看</el-button>
            </div>
          </div>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    // 跳转大棚界面
    chg_gh (list) {
      sessionStorage.setItem('activeName', list.greenHouseId)
    ...
    myGreenList () {
      ...
      this.axios.post(this.$store.state.ip + 'gh/myGreenHouse', formdata).then(res => {
        ...
        this.activeName = this.listGreen[0].greenHouseId
      }).catch(err => {
        console.log(err)
      })
    },
